...
void read_image_template(char *name, float **image, int *im_width, int *im_height);
void write_image(char *name, unsigned char *image, int im_width, int im_height);
void write_image_template(char *name, float *image, int im_width, int im_height);
...
void read_image_template(char *name, float **image, int *im_width, int *im_height)
{
	...
	int i;
        ...
        *image=(float *)malloc(sizeof(int)*(*im_width)*(*im_height));


        for(i=0;i<(*im_width)*(*im_height);i++)
        {
                (*image)[i]=(float)temp_img[i];
        }
        free(temp_img);
}
...
void write_img_template(char *name, float *image,int im_width,int im_height)
{
