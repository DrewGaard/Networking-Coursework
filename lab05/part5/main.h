...
void convolve(float *org_img, int img_width, int img_height, float *mask, int gauss_width, float *op_img)
{
	int i, j, k, l;
	float temp;
...
//double Thresholding
void double_threshold(float *Gxy,int thigh,int tlow,int width,int height)
{
        int i,j;
        float pixel;
        for(i=0;i<height;i++)
                for(j=0;j<width;j++)
                        if(Gxy[i*width+j]>thigh)
                                Gxy[i*width + j]= (float)255;
                        else if (Gxy[i*width+j]<thigh && Gxy[i*width+j]>tlow)
                                Gxy[i*width + j]= (float)125;
                        else
                                Gxy[i*width + j]= (float)0;

}


void edge_linking(float *thresh, float *hyst , int img_width, int img_height)
{

	int i,j; //variables were undeclared
	
	memcpy(hyst,thresh,sizeof(float)*img_width*img_height); //VKP: This memcpy was missing too

	for(i = 0; i < img_height; i++)
	{
		for(j = 0; j < img_width; j++)
		{
			if(thresh[i * img_width + j] == 125)
			{
				if((j-1)>=0)
					if(thresh[i*img_width + (j-1)] == 255)
						hyst[i*img_width + j] = 255;
				else if(j+1 < img_width)
					if(thresh[i*img_width + (j+1)] == 255)
						hyst[i*img_width + j] = 255;
					else if(i-1 >= 0 && j-1 >= 0)
						if(thresh[(i-1)*img_width + (j-1)] == 255)
							hyst[i*img_width + j] = 255;
					else if(i+1 < img_height && j+1 < img_width)
						if(hyst[(i+1)*img_width + (j+1)] == 255)
							hyst[i*img_width + j] = 255;
					else if(i-1 >= 0)
						if(thresh[(i-1)*img_width + (j)] == 255)
							hyst[i*img_width + j] = 255;
					else if(i+1 < img_height)
						if(thresh[(i+1)*img_width + (j)] == 255)
							hyst[i*img_width + j] = 255;
					else if(i+1 < img_height)
						if(thresh[(i+1)*img_width + (j-1)] == 255)
							hyst[i*img_width + j] = 255;
					else if(i-1 >= 0 && j+1 < img_width)
						if(thresh[(i-1)*img_width + (j+1)] == 255)
							hyst[i*img_width + j] = 255;
					else
						hyst[i*img_width + j] = 0;
			}
		}
	}
}
